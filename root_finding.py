...
"""
1변수 방정식의 해
어떤 (비선형) 함수 f(x) 값이 0 이 되도록 만드는 x를 찾음
"""
...
def bisection(f, xl, xh, epsilon=epsilon_global, b_verbose=False):
    ...
    f(xl) 과 f(xn) 의 부호가 반대인 xl, xh 에서 시작
    ...
    :param xl: x 의 초기값 xl < xh && f(xl) f(xh) < 0
    :param xh: x 의 초기값 xl < xh && f(xl) f(xh) < 0
    :param epsilon: 오차 허용 범위
    ...
    while True:
        ...
        if f(xn) * f(xh) < 0:
            ...
            # 같으면 : 근이 xl~ xn 사이에 있음. xh에 xn 을 저장
            xh = xn
        ...
        # xl ~xh 구간의 길이가 epsilon 보다 짧으면 무한 반복뮨울 중단
...
def newton(f, df, x0, epsilon=epsilon_global, b_verbose=False):
    ...
    접선의 방정시구 di (x - xi) + fi 는 x = xi - fi/di 이면 0이 됨
    ...
    while True:
        ...
        #반복문이 한번 실행 되엇으므로 counter 를 1 증가 시킴
        counter += 1
...
def func(x):
    ...
    이 경우는 x * x -2.0 = 0 을 만족하는 x를 찾게되며 이러한 x 는 2 ** 0.5 즉 2의 제곱근임
    :param x:
    :return:
    """
    return 1.0 * x * x - 2.0
...
# end of func()
# BATzerk, "Square Root Calculatro", Scratch, [Online] Available : https://scratch.mit.edu/prouects/35488/
